    ...
    function (moduleArg = {}) {
          ...
          createPeer(sock, addr, port) {
            ...
            if (ws) {
              if (ws._socket) {
                ...
                var runtimeConfig = Module["websocket"] &&
                ...
                if (
                  runtimeConfig && null === Module["websocket"]["subprotocol"]
                ) {
                  subProtocols = "null";
                  opts = undefined;
                }
        ...
        if (mode == 0) {
          Browser.mainLoop.scheduler =
            function Browser_mainLoop_scheduler_setTimeout() {
              var timeUntilNextTick = Math.max(
                0,
                Browser.mainLoop.tickStartTime + value -
                  _emscripten_get_now(),
              ) | 0;
      ...
      var GL = {
        ...
        createOffscreenFramebuffer: (context) => {
          ...
          context.blitVB = vb;
          var vsCode = "attribute vec2 pos;" + "varying lowp vec2 tex;" +
            "void main() { tex = pos * 0.5 + vec2(0.5,0.5); gl_Position = vec4(pos, 0.0, 1.0); }";
          var vs = gl.createShader(35633);
          gl.shaderSource(vs, vsCode);
          gl.compileShader(vs);
          var fsCode = "varying lowp vec2 tex;" + "uniform sampler2D sampler;" +
      ...
      var Browser = {
          ...
          if (canvas) {
            ...
            if (Module["elementPointerLock"]) {
              canvas.addEventListener("click", (ev) => {
                if (
                  !Browser.pointerLock && Module["canvas"].requestPointerLock
                ) {
        ...
        createContext(canvas, useWebGL, setInModule, webGLContextAttributes) {
          ...
          if (useWebGL) {
            var contextAttributes = {
              antialias: false,
              alpha: false,
              majorVersion: typeof WebGL2RenderingContext != "undefined"
                ? 2
                : 1,
            };
        ...
        updateCanvasDimensions(canvas, wNative, hNative) {
          ...
          var h = hNative;
          if (
            Module["forcedAspectRatio"] && Module["forcedAspectRatio"] > 0
          ) {
      ...
      var JSEvents = {
        ...
        registerOrRemoveHandler(eventHandler) {
          ...
          } else {for (var i = 0; i < JSEvents.eventHandlers.length; ++i) {
              if (
      ...
      var _glCompressedTexImage3D = (
        ...
        } else {GLctx.compressedTexImage3D(
            target,
            level,
            internalFormat,
            width,
      ...
      var _glCompressedTexSubImage3D = (
        ...
        } else {GLctx.compressedTexSubImage3D(
            target,
            level,
            xoffset,
            yoffset,
          ...
          switch (typeof result) {
            ...
            case "object":
              if (result === null) {
                ...
                result instanceof Uint32Array || result instanceof Int32Array ||
      ...
      var _glGetShaderiv = (shader, pname, p) => {
        ...
        } else {HEAP32[p >> 2] = GLctx.getShaderParameter(
            GL.shaders[shader],
            pname,
          );}
      };
      ...
      var _glGetUniformLocation = (program, name) => {
        ...
        if (program = GL.programs[program]) {
          ...
          if (sizeAndId && arrayIndex < sizeAndId[0]) {
            ...
            if (
              uniformLocsById[arrayIndex] = uniformLocsById[arrayIndex] ||
      ...
      var heapObjectForWebGLType = (type) => {
        ...
        if (
          type == 5 || type == 28922 || type == 28520 || type == 30779 ||
          type == 30782
        ) return HEAPU32;
        return HEAPU16;
      };
        ...
        init: function (persistentPaths) {
          ...
          function createRecursive(dir) {
            try {
              ...
              if (e.errno !== GodotFS.ENOENT) GodotRuntime.error(e);
              FS.mkdirTree(dir);
            }
          }
      ...
      var GodotAudio = {
        ...
        SampleNodeBus: class SampleNodeBus {
          ...
          setVolume(volume) {
            ...
            this._sl.gain.value = volume[GodotAudio.GodotChannel.CHANNEL_SL] ??
              0;
            this._sr.gain.value = volume[GodotAudio.GodotChannel.CHANNEL_SR] ??
              0;
        ...
        SampleNode: class SampleNode {
          ...
          constructor(params, options = {}) {
            ...
            this.loopMode = options.loopMode ?? this.getSample().loopMode ??
              "disabled";
            this._pitchScale = 1;
          ...
          _restart() {
            if (this._source != null) this._source.disconnect();
            this._source = GodotAudio.ctx.createBufferSource();
            this._source.buffer = this.getSample().getAudioBuffer();
            for (const sampleNodeBus of this._sampleNodeBuses.values()) {
        ...
        Bus: class Bus {
          ...
          static getBusOrNull(index) {
            if (index < 0 || index >= GodotAudio.buses.length) return null;
            return GodotAudio.buses[index];
          }
          ...
          static create() {
            ...
            if (isFirstBus) newBus.setSend(null);
            else newBus.setSend(GodotAudio.Bus.getBus(0));
            return newBus;
          }
          constructor() {
          ...
          setVolumeDb(val) {
            ...
            if (isFinite(linear)) this._gainNode.gain.value = linear;
          }
          getSend() {
            return this._send;
          }
          ...
          _enableSolo() {
            ...
            const otherBuses = GodotAudio.buses.filter((otherBus) =>
              otherBus !== this
            );
      ...
      var GodotAudioWorklet = {
        ...
        create: function (channels) {
          ...
          GodotAudioWorklet.promise = GodotAudio.ctx.audioWorklet.addModule(
            path,
          ).then(function () {
      ...
      var GodotEventListeners = {
        ...
        has: function (target, event, method, capture) {
          return GodotEventListeners.handlers.findIndex(function (e) {
            return e.target === target && e.event === event &&
              e.method === method && e.capture === capture;
          }) !== -1;
        },
          ...
          try {
            const promise =
              (canvas.requestFullscreen || canvas.msRequestFullscreen ||
                ...
                canvas.webkitRequestFullscreen).call(canvas);
      ...
      var GodotFetch = {
        ...
        create: function (method, url, headers, body) {
          ...
          obj.request.then(GodotFetch.onresponse.bind(null, id)).catch(
            GodotFetch.onerror.bind(null, id),
          );
          return id;
        },
        free: function (id) {
        ...
        read: function (id) {
          ...
          if (obj.reader && !obj.reading) {
            ...
            obj.reading = true;
            obj.reader.read().then(GodotFetch.onread.bind(null, id)).catch(
              GodotFetch.onerror.bind(null, id),
            );
          }
        },
      ...
      var GodotIME = {
        ...
        getModifiers: function (evt) {
          return evt.shiftKey + 0 + (evt.altKey + 0 << 1) +
            (evt.ctrlKey + 0 << 2) + (evt.metaKey + 0 << 3);
        },
      ...
      function _godot_js_os_has_feature(p_ftr) {
        ...
        const ua = navigator.userAgent;
        if (ftr === "web_macos") return ua.indexOf("Mac") !== -1 ? 1 : 0;
        if (ftr === "web_windows") return ua.indexOf("Windows") !== -1 ? 1 : 0;
        if (ftr === "web_android") return ua.indexOf("Android") !== -1 ? 1 : 0;
        if (ftr === "web_ios") {
        ...
        if (ftr === "web_linuxbsd") {
          return ua.indexOf("CrOS") !== -1 || ua.indexOf("BSD") !== -1 ||
              ua.indexOf("Linux") !== -1 || ua.indexOf("X11") !== -1
            ? 1
            : 0;
        }
        return 0;
      ...
      function _godot_js_pwa_cb(p_update_cb) {
        if ("serviceWorker" in navigator) {
          ...
          navigator.serviceWorker.getRegistration().then(
            GodotPWA.updateState.bind(null, cb),
          );
        }
      }
      ...
      var GodotRTCPeerConnection = {
        ...
        create: function (
          ...
          if (
            "connectionState" in conn && conn["connectionState"] !== undefined
          ) {
            conn.onconnectionstatechange = function (event) {
              if (!IDHandler.get(id)) return;
              onConnectionChange(
                GodotRTCPeerConnection.ConnectionState[conn.connectionState] ||
                  0,
              );
            };
          } else {conn.oniceconnectionstatechange = function (event) {
              if (!IDHandler.get(id)) return;
              onConnectionChange(
                GodotRTCPeerConnection
                  .ConnectionStateCompat[conn.iceConnectionState] || 0,
              );
            };}
          conn.onicegatheringstatechange = function (event) {
            ...
            onIceGatheringChange(
              GodotRTCPeerConnection
                .IceGatheringState[conn.iceGatheringState] || 0,
            );
          };
      ...
      function _godot_js_rtc_pc_local_description_set(
        ...
        ref.setLocalDescription({ sdp: sdp, type: type }).catch(
          function (error) {
      ...
      function _godot_js_wrapper_create_cb(p_ref, p_func) {
        ...
        let id = 0;
        const cb = function () {
          if (!GodotJSWrapper.get_proxied_value(id)) return undefined;
      ...
      var GodotWebXR = {
        ...
        getTextureId: (texture) => {
          if (texture.name !== undefined) return texture.name;
        ...
        getInputSourceId: (input_source) => {
          if (input_source !== undefined) return input_source.name;
          return -1;
        },
      ...
      function _godot_webxr_get_projection_for_view(p_view, r_transform) {
        if (!GodotWebXR.session || !GodotWebXR.pose) return false;
      ...
      function _godot_webxr_get_transform_for_view(p_view, r_transform) {
        ...
        let matrix;
        if (p_view >= 0) matrix = views[p_view].transform.matrix;
        else matrix = GodotWebXR.pose.transform.matrix;
        for (let i = 0; i < 16; i++) {
          GodotRuntime.setHeapValue(r_transform + i * 4, matrix[i], "float");
        }
        return true;
      ...
      var _godot_webxr_initialize = function (
        ...
        navigator.xr.requestSession(session_mode, session_init).then(
          function (session) {
            ...
            gl.makeXRCompatible().then(function () {
              ...
              function onReferenceSpaceSuccess(
                ...
                window.setTimeout(function () {
                  const reference_space_c_str = GodotRuntime.allocString(
                    reference_space_type,
                  );
                  const enabled_features = "enabledFeatures" in session
              ...
              function requestReferenceSpace() {
                const reference_space_type = requested_reference_space_types
                  .shift();
                session.requestReferenceSpace(reference_space_type).then(
                  (refSpace) => {
      ...
      function _godot_webxr_is_session_supported(p_session_mode, p_callback) {
        ...
        if (navigator.xr) {
          navigator.xr.isSessionSupported(session_mode).then(
            function (supported) {
      ...
      for (var i = 0; i <= 288; ++i) {
        miniTempWebGLFloatBuffers[i] = miniTempWebGLFloatBuffersStorage
          .subarray(0, i);
      }
      ...
      var __Z14godot_web_mainiPPc = Module["__Z14godot_web_mainiPPc"] = (
        ...
        (__Z14godot_web_mainiPPc =
          Module["__Z14godot_web_mainiPPc"] =
      ...
      var __emwebxr_on_simple_event = Module["__emwebxr_on_simple_event"] = (
        ...
        (__emwebxr_on_simple_event =
          Module["__emwebxr_on_simple_event"] =
            wasmExports["wf"])(a0);
